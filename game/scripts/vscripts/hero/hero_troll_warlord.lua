...
function modifier_imba_berserkers_rage_passive:OnAttackLanded(keys)
	if IsServer() and self:GetParent() == keys.attacker and self:GetParent().splitattack and keys.target:IsAlive() and not self:GetParent():PassivesDisabled() then
		if PseudoRandom:RollPseudoRandom(self:GetAbility(), self:GetAbility():GetSpecialValueFor("bash_chance")) and (keys.target:IsHero() or keys.target:IsCreep()) and self:GetParent():IsTrueHero() then
...
function modifier_imba_battle_trance:OnCreated() self.batreduc = (self:GetParent():GetBaseAttackTime() - self:GetParent():GetBaseAttackTime() * (self:GetAbility():GetSpecialValueFor("bat_scepter") / 100)) end
