...
function modifier_imba_enigma_black_hole_thinker:OnCreated()
	if IsServer() then
		...
		local enemies2 = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetParent():GetAbsOrigin(), nil, 250000, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false)
...
function modifier_imba_enigma_black_hole_aura:OnCreated()
	if IsServer() then
		...
		self:AddParticle(pfx, false, false, 15, false, false)
		PlayerResource:SetCameraTarget(self:GetParent():GetPlayerID(), self:GetParent())
	end
end
