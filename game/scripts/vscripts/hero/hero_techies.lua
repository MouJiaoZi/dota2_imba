...
function imba_techies_land_mines:OnSpellStart()
	...
	mine:AddNewModifier(caster, self, "modifier_techies_remote_mine", {duration = self:GetSpecialValueFor("duration")})
end
	...
	if IsServer() then
		if self:GetElapsedTime() >= self:GetDuration() then
			...
			if land_mine and land_mine:GetLevel() > 0 then
				...
				mine:AddNewModifier(caster, land_mine, "modifier_techies_remote_mine", {duration = land_mine:GetSpecialValueFor("duration")})
			end
			...
			if stasis_trap and stasis_trap:GetLevel() > 0 then
				...
				mine:AddNewModifier(caster, stasis_trap, "modifier_techies_remote_mine", {duration = stasis_trap:GetSpecialValueFor("duration")})
			end
			...
			if remote_mine and remote_mine:GetLevel() > 0 then
				...
				mine:AddNewModifier(parent, remote_mine, "modifier_techies_remote_mine", {duration = remote_mine:GetSpecialValueFor("duration")})
...
function imba_techies_minefield_sign:OnSpellStart()
	...
	if not self.sign then
		...
		sign:AddNewModifier(caster, self, "modifier_techies_remote_mine", {})
		self.sign = sign
	else
...
function imba_techies_remote_mines:OnAbilityPhaseStart()
	...
	self.mine:AddNewModifier(caster, self, "modifier_techies_remote_mine", {duration = 3.0})
