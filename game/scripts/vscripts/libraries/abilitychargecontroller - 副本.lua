...
function modifier_imba_ability_charge:OnIntervalThink()
	...
	elseif self.ability:IsCooldownReady() or self.ability:GetCooldownTimeRemaining() > self:GetRemainingTime()+0.03 then
		self.ability:EndCooldown()
...
function modifier_imba_ability_charge:OnAbilityFullyCast( keys )
	...
	if keys.ability == self.ability then
		AbilityChargeController:CostCharge(self.ability)
	end
end
...
function AbilityChargeController:ChangeChargeAbilityConfig(hAbility, fChargeTime, iMaxCharges, iChargeCost, bResetByRefresh, bCDR)
	...
	buff.maxcharges = iMaxCharges or buff.chargetime 
	buff.chargecost = iChargeCost or buff.chargetime 
	buff.refresh = bResetByRefresh or buff.chargetime 
	buff.getcdr = bCDR or buff.chargetime
end
